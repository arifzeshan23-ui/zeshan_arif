...
os.makedirs(os.path.join(uploads_path, "images"), exist_ok=True)
os.makedirs(os.path.join(uploads_path, "cv"), exist_ok=True)
...
def startup():
    try:
        ...
        print(f"Startup DB init skipped (MySQL not available?): {e}")
        print("The server will still run — make sure MySQL is configured.")
